...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        -- if love.mousepressed(1) then
        --     self.pressed = true
        --     self.mouseX, self.mouseY = love.mouse.getPosition()
        --     self.mouseX, self.mouseY = push:toGame(self.mouseX, self.mouseY)

        --     self.boardHighlightX = math.floor(self.mouseX)
        --     self.boardHighlightY = math.floor(self.mouseY)
        -- else
        --     self.pressed = false
        -- end
        ...
        if love.keyboard.wasPressed("enter") or love.keyboard.wasPressed("return") then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                Timer.tween(
                    ...
                    -- once the swap is finished, we can tween falling blocks as

                    function()
                        self:checkMatches()
                        if self.viableMove then
    ...
    if self:canMove() then
        ...
        Timer.tween(0.5, tilesToFall):finish(
            function()
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            -- iterate through the match and increase score based on tile variety
...
function PlayState:checkMatches()
    ...
    if matches then
        ...
        self.viableMove = false
    end
end

function PlayState:canMove()
    for x = 1, 8 do
        for y = 1, 8 do
            local tile = self.board.tiles[y][x]
            local newTile
            if x > 1 and x < 8 then
                newTile = self.board.tiles[y][x + 1]
                self:swapTiles(tile, newTile)
                self:checkMatches()
                if (self.viableMove) then
                    self:swapTiles(tile, newTile)
                    return true
                ...
                if y > 1 and y < 8 then
                    newTile = self.board.tiles[y + 1][x]
                    self:swapTiles(tile, newTile)
                    self:checkMatches()
                    if (self.viableMove) then
                        self:swapTiles(tile, newTile)
...
function PlayState:render()
    ...
    --     love.graphics.printf("mouse pressed", 0, VIRTUAL_HEIGHT / 2 + 33, VIRTUAL_WIDTH, "center")
    --     love.graphics.printf("MouseX " .. tostring(self.mouseX), 0, VIRTUAL_HEIGHT / 2 + 45, VIRTUAL_WIDTH, "center")
    --     love.graphics.printf("mouseY " .. tostring(self.mouseY), 0, VIRTUAL_HEIGHT / 2 + 55, VIRTUAL_WIDTH, "center")
