...
function GenerateFlags(atlas)
    ...
    local sheetCounter = 1
    for i = 0, 4 do
        spritesheet[sheetCounter] = love.graphics.newQuad(i * 16, 0, 16, 48, atlas:getDimensions())
        sheetCounter = sheetCounter + 1
    end
...
function GenerateFlagCloths(atlas)
    local spritesheet = {}
    local sheetCounter = 1

    for y = 0, 4 do
        for x = 0, 2 do
            spritesheet[sheetCounter] = love.graphics.newQuad(96 + (x * 16), y * 14, 16, 14, atlas:getDimensions())
            sheetCounter = sheetCounter + 1
        end
    end
    return spritesheet
