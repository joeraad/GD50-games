--[[
    ...
    PlayState Class is used to represent the state of the game when the player is playing the game.
...
function PlayState:updateCamera()
    ...
    self.camX =
        math.max(0, math.min(TILE_SIZE * self.tileMap.width - VIRTUAL_WIDTH, self.player.x - (VIRTUAL_WIDTH / 2 - 8)))

    -- adjust background X to move a third the rate of the camera for parallax
    self.backgroundX = (self.camX / 3) % 256
end

function PlayState:enter(player)
    if player == nil then
    else
