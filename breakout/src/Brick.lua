...
function Brick:hit()
    -- special case for locked brick if the player doesn't have the key
    if self.locked == true and not gHasKey then
        ...
        -- special case for locked brick if the player has the key
        if (self.locked == true and gHasKey) then
            gHasKey = false
            self.locked = false
            self.psystem:setColors(
                paletteColors[7].r,
            ...
            self.psystem:emit(64)
            gSounds["lock-hit-unlock"]:stop()
            gSounds["lock-hit-unlock"]:play()

            -- change the locked brick to a random block type and tier
        ...
        -- 25% chance to spawn powerup
        if math.random() < 0.25 then
            Powerup:spawn(self.x, self.y)
        end
    end
end
